...
export const PLAN_LIMITS: Record<string, { accounts: number; scheduledPosts: number; aiCredits: number }> = {
...
const getLimits = (planType: string) =>
    PLAN_LIMITS[planType] || PLAN_LIMITS.starter;

export const getAccountPlanStatus = async (userId: string, planType: string = "starter") => {
...
export const getPostPlanStatus = async (userId: string, planType: string = "starter") => {
    const limits = getLimits(planType);
